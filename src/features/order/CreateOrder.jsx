...
function CreateOrder() {
  const {username,status:addresStatus,position,address,error:errorAddress,} = useSelector((state)=>state.user);
  ...
  const priorityPrize = withPriority ? totalCartPrice * 0.2 : 0;
  const totalPrize = totalCartPrice + priorityPrize;
        ...
        <div className='mb-5 flex gap-2 flex-col sm:flex-row sm:items-center relative'>
          ...
          <div className='grow'>
            <input
              ...
              required
            />
            {addresStatus === 'error' && (<p className='text-xs mt-2 text-red-700 bg-red-100 rounded-md px-2 py-2'>{errorAddress}</p>)}
          </div>
